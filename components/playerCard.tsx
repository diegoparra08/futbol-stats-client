  ...
  >
    ...
    <div className="flex justify-between items-center w-full">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className={`text-slate-500 text-xs transform transition-transform duration-300 ${isOpen ? "rotate-180 text-emerald-400" : ""}`}>
          ▼
        </span>
      </div>
    </div>
      ...
      {loadingStats ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs text-slate-300">
          ...
          <div className="bg-slate-950/80 border border-slate-800/40 p-2.5 rounded-lg">
            <p className="text-slate-500 font-medium mb-0.5">Partidos Jugados</p>
            <p className="text-sm font-bold text-emerald-400">{stats.matchesPlayed ?? 0}</p>
          </div>

          {/* Cuadrado de Goles */}
          <div className="bg-slate-950/80 border border-slate-800/40 p-2.5 rounded-lg">
            <p className="text-slate-500 font-medium mb-0.5">Goles Totales</p>
            <p className="text-sm font-bold text-slate-200">{stats.goals ?? 0}</p>
          </div>
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between items-center text-xs">
        ...
        <span className={`font-bold px-1.5 py-0.5 rounded text-[11px] bg-slate-950 border border-slate-800 text-slate-200`}>
          {numValue}
        </span>
      </div>
